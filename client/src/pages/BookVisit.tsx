...
import { Booking, Pet } from "@shared/schema";
...
export default function BookVisit() {
  ...
  const { data: pets, isLoading: isPetsLoading } = useQuery<Pet[]>({ queryKey: ["/api/pets"] });
  ...
  const bookingMutation = useMutation({
    ...
    onError: () => {
      toast({
        title: "Error",
        description: "Failed to send visit request. Please try again.",
        variant: "destructive"
      });
    }
  });
            ...
            <div className="grid md:grid-cols-5 h-full">
              ...
              <div className="p-8 md:col-span-3 bg-white">
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <Button type="submit" className="w-full h-14 text-lg font-bold rounded-xl mt-2" disabled={bookingMutation.isPending}>
                    {bookingMutation.isPending ? "Scheduling..." : "Confirm Visit Request"}
                  </Button>
                </form>
              </div>
            </div>
